function y = GenerateMicData_V3(baseSound, fs, position_nodes, ...
    sound_location, amplituteOffset, noisePM, timeVar, type)
...
n_samples = size(baseSound, 1);                                 % sound n_samples in samples
n_nodes = size(position_nodes,1);
y = zeros(n_samples, n_channels, size(position_nodes, 1));      % Create empty 3D y matrix dim(samples, channels, position_nodes)
...
for i_loc = 1:size(sound_location, 1)
    ...
    for i_node = 1:size(position_nodes, 1)
        ...
        y_i = zeros(n_samples, n_channels);% 2D y matrix (samples, channels) as it would be captured on the device
...
% printing progress
n_iterations = 2*n_samples*size(sound_location, 1);
current_it = 0;
print_factor = floor(n_iterations/100);
print_it = 0;
...
s1 = 100;
s2 = 6000;
sync = zeros(2*n_samples,1);
syncVal = 0;
rand_ctr = 0;

for i_sample=1:2*n_samples*size(sound_location, 1)            % For all samples (twice because of time var offsets)
    % print progress
    print_it = print_it + 1;
    if print_it == print_factor
        print_it = 0;
...
for i_node = 1:size(position_nodes, 1)
    %i_tmp = i_tmp+1
    i_sync = sync(((i_loc-1) * (n_samples)) + 1 + floor(timeVar(i_node)*fs) : (i_loc) * (n_samples) + floor(timeVar(i_node)*fs));
    y(:,6,i_node) = i_sync;
end
...
n_batch = n_samples/samples_per_batch;
