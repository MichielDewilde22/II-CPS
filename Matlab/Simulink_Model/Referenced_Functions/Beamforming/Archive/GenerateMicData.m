function y = GenerateMicData(baseSound, fs, position_nodes, ...
...
for i_loc = 1:size(sound_location, 1) 
    ...
    for i_node = 1:size(position_nodes, 1)
        ...
        for i_mic=1:n_channels-1 
            ...
            capture_A = capture(end+sample_diff+1:end);
            capture = [capture_A; capture(1:end+sample_diff)];

            y_i(soundIdxs(i_loc) : soundIdxs(i_loc+1),i_mic) = capture(soundIdxs(i_loc) : soundIdxs(i_loc+1));
